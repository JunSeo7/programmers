...
SELECT 
    EMP_NAME    이름
    , JOB_CODE  "직급 코드"
    , TO_CHAR(SALARY, '99,999,999') || '원'    급여
    , TO_CHAR(TO_DATE(HIRE_DATE),'YYYY-MM-DD') 입사일
FROM EMPLOYEE
WHERE SALARY =
(
    SELECT MIN(SALARY)
...
--3) 노옹철 사원의 급여보다 더 많은 급여를 받는 사원들
SELECT 
    EMP_ID
    , EMP_NAME
    , DEPT_TITLE
...
-- 4) 과장 직급임에도 차장 직급의 최대 급여보다 
--    더 많이 받는 직원들의 사번, 이름, 직급명, 급여 조회
SELECT 
    EMP_ID
    , EMP_NAME
    , JOB_NAME
...
-- 2) 박나라 사원과 직급 코드가 일치하면서 같은 
--    사수를 가지고 있는 사원의 사번, 이름, 직급 코드, 사수 사번, 조회
SELECT *
FROM EMPLOYEE
