...
FROM DUAL; -- 3번째 자리의 B의 위치값 출력
SELECT INSTR('AABAACAABBAA', 'B', 1) 
FROM DUAL; -- 3번째 자리의 B의 위치값 출력
SELECT INSTR('AABAACAABBAA', 'B', 1, 2) 
FROM DUAL; -- 9번째 자리의 B의 위치값 출력
SELECT INSTR('AABAACAABBAA', 'B', -1) 
FROM DUAL; -- 10번째 자리의 B의 위치값 출력
...
SELECT LTRIM(' 123123KH', '12 ') FROM DUAL;
...
-- EMPLOYEE 테이블에서 주민등록번호 첫 번째 자리부터 성별까지를 추출한 결과값 
-- 오른쪽에 * 문자를 채워서 조회 (991212-2****** 형태로 출력)
SELECT RPAD(SUBSTR(EMP_NO, 1, 8),14, '*') 주민등록번호
FROM EMPLOYEE
;

SELECT SUBSTR(EMP_NO, 1, 8) || '******' 주민등록번호
FROM EMPLOYEE
;

-- EMPLOYEE 테이블에서 사원명, 이메일, 아이디(이메일에서 '@' 앞의 문자 값만 출력)를 조회
SELECT EMP_NAME "이름 ", SUBSTR(EMAIL, 1, INSTR(EMAIL, '@',1)-1) ID , EMP_ID 사원번호
FROM EMPLOYEE
;
...
SELECT EMP_NAME || '의 사원 번호 : ' || EMP_ID
FROM EMPLOYEE;
...
/*
    3) ROUND
        - ROUND(NUMBER[, 위치])
        - 위치를 지정하여 반올림해주는 함수
        - 위치 : 기본값 0(.), 양수(소수점 기준으로 오른쪽)와 
                음수(소수점 기준으로 왼쪽)로 입력가능
*/
...
SELECT TO_CHAR(SYSDATE, 'MM.DD HH.MI') FROM DUAL;
...
SELECT 
    EMP_NAME
    , DEPT_CODE
    , DECODE(DEPT_CODE, 'D1', '개발1팀', 'D2','개발2팀', 'D5', '개발3팀') AS 부서명
FROM EMPLOYEE;

-- 사원명, 직급코드, 직급명 조회 (직급명 상관 X)
SELECT 
    EMP_NAME
    , JOB_CODE
    , DECODE(
